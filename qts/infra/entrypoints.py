...
def resolve_entry_config_path(config_path: str | Path | None = None) -> Path | None:
    """解析统一入口使用的配置路径。"""
...
def _load_config(config_path: str | Path | None = None) -> tuple[Path | None, QTSConfig]:
    """加载统一入口配置。"""
    resolved = resolve_entry_config_path(config_path)
...
def build_run_summary(run: EntryRun) -> str:
    """生成入口运行摘要。"""
    return (
...
def save_entry_artifacts(run: EntryRun) -> None:
    """按配置写出入口产物。"""
    writers = {
        ...
        "run_summary": lambda: save_text(build_run_summary(run), run.artifact_dir / "run.txt"),
    }
