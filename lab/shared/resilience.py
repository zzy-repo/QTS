...
from .execution import ExecutionRun, execute_rebalance, dynamic_slippage_cost
...
def expand_to_ticks(close: pd.DataFrame, bars_per_day: int = 3) -> pd.DataFrame:
    ...
    for i, (ts, row) in enumerate(close.iterrows()):
...
def build_strategy_fleet(
    builder: Callable[[int], pd.DataFrame],
    count: int,
) -> pd.DataFrame:
